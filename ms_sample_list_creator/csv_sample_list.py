...
class csvBatch(tk.Frame):
    ...
    def _initialize_env_variables(self) -> None:
        ...
        if env_method_files:
            self.tk_variables.method_files = [
                str(k) for k in env_method_files.split(",") if k
            ]
    ...
    def _load_and_process_csv(self, file_path: Optional[str]) -> pd.DataFrame:
        ...
        # Change file name
        df["File Name"] = df["File Name"].apply(lambda x: "_".join([self.timestamp, self.tk_vars.operator_settings.operator.get()] + x.split("_")[2:]))
        return df
    ...
    def _prepare_data_for_directus(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        directus_df["instrument_used"] = self.session_data.instrument_key  # Utilisation de DirectusSessionData
        directus_df["batch"] = self.session_data.batch_key  # Utilisation de DirectusSessionData
    ...
    def _add_parent_sample_container(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for index, row in df.iterrows():
            aliquot_id = "_".join(row["filename"].split("_")[2:])
    ...
    def _send_data_to_directus(self, df: pd.DataFrame) -> None:
        ...
        headers = {"Content-Type": "application/json", "Authorization": f"Bearer {self.session_data.access_token}"}  # Utilisation de DirectusSessionData
    ...
    def _write_blanks(self, csv_writer: Any, blk_type: str) -> None:
        ...
        blk_count = getattr(self.tk_vars, f"{blk_type}_blk").get()
        ...
        for i in range(1, blk_count + 1):
            ...
            filename = f"{self.timestamp}_{self.tk_vars.operator_settings.operator.get()}_emi_{self.tk_vars.blk_name.get()}_blk_{blk_type}{padded_number}"
            ...
            csv_writer.writerow([
                ...
                self.tk_vars.blk_pos.get(),  
                self.tk_vars.inj_volume.get()  
            ])

    def _write_standby_line(self, csv_writer: Any)-> None:
        """Write standby line to CSV."""
        parts = self.tk_vars.standby_file.get().split("/")  # Utilisation de la dataclass TkVariables
        filename = f"{self.timestamp}_{self.tk_vars.operator_settings.operator.get()}_{parts[-1]}"
        csv_writer.writerow([filename, self.tk_vars.data_path.replace("/", "\\"), self.tk_vars.standby_file.get().replace("/", "\\"), self.tk_vars.blk_pos.get(), self.tk_vars.inj_volume.get()])
